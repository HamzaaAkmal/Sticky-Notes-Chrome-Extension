(function() {
    ...
    function loadNotes() {
        try {
            ...
            chrome.storage.sync.get(urlKey, (data) => {
                ...
                notes.forEach((note) =>
                    createStickyNote(note.content, note.top, note.left, note.collapsed, note.title, note.color, note.id)
                );
            });
        } catch (error) {
    ...
    function saveNotes() {
        try {
            const notes = Array.from(document.querySelectorAll(".sticky-note")).map(
                (note) => {
                    // Get existing ID from dataset
                    let noteId = note.dataset.noteId;
                    
                    // If no ID exists, generate one now and save it to the element
                    if (!noteId || noteId.trim() === '') {
                        const content = note.querySelector(".sticky-content").innerHTML;
                        ...
                        const key = `${content}_${top}_${left}_${title}`;
                        const hash = key.split('').reduce((acc, char) => {
                            return ((acc << 5) - acc) + char.charCodeAt(0);
                        }, 0);
                        noteId = `note_${Math.abs(hash)}_${Date.now()}`;
                        
                        // Store it in dataset for next time
            ...
            chrome.storage.sync.set({ 
                ...
                if (chrome.runtime.lastError) {
                    ...
                    console.log('Notes saved successfully, auto-sync will trigger');
                }
            });
        } catch (error) {
    ...
    function createStickyNote(content = "", top = null, left = null, collapsed = false, title = "Note", color = "#ffd165", id = null) {
        try {
            const note = document.createElement("div");
            note.className = "sticky-note";
            note.dataset.color = color;
            note.dataset.noteId = id || `note_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
